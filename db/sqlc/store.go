...
func (store *Store) TransferTx(ctx context.Context, arg TransferTxParams) (TransferTxResult, error) {
	...
	err := store.execTx(ctx, func(q *Queries) error {
		...
		account1, err := q.GetAccountForUpdate(ctx, arg.FromAccountID) // Получаем запись о счете и присваиваем переменной account
		if err != nil {
			return err
		}

		result.FromAccount, err = q.UpdateAccount(ctx, UpdateAccountParams{ // Обновляем баланс счета
